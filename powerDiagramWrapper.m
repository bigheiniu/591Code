function [PD, PDinf] = powerDiagramWrapper(E, wts,color)
...
if (nT-1==2)
    ...
    plot(PC(:,1), PC(:,2), 'r.');
    hold on
    powerdiagram(E(:,1), E(:,2), T, color, wts);
    plot(E(:,1), E(:,2), 'b.');
    hold on;
    % output point name 
    robotnumber = size(E);
    ...
    for i=1:robotNumber
    % draw line and name of point
    %text(E(i,1),E(i,2),[num2str(i),'-w = ',num2str(wts(i))],'FontSize',10,'Color','red');
    text(E(i,1),E(i,2),[num2str(i)],'FontSize',10,'Color','red');
    end
end
end